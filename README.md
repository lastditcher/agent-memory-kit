...
# 方式 1: 只安装骨架（不装 RAG，适合不需要语义检索的项目）
python agent-memory-kit/scripts/setup.py ./docs --name "MyProject" --tech "Python/FastAPI"

# 方式 2: 安装骨架 + RAG（自动安装 sentence-transformers，一步到位）
python agent-memory-kit/scripts/setup.py ./docs --name "MyProject" --tech "Python/FastAPI" --with-rag
```
